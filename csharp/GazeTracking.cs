...
public class GazeTracking : MonoBehaviour
{
    ...
    bool flag = false; // Flag indicating whether the user is gazing at the marker
    int gaze_flag = 0; // Flag indicating the marker number
    float init_time = 0.0f; // Start time of the gazing
    int marker_num = 0;
    int[] marker_list = {0, 6, 2, 19, 10, 14, 20, 7, 25, 8, 22, 1, 12, 23, 5, 15, 16, 4, 18, 3, 11, 21, 17, 13, 9, 24};
    ...
    public GameObject controller; // Controller object
    private SteamVR_Action_Boolean actionToTrigger = SteamVR_Actions._default.InteractUI;
    ...
    void FixedUpdate()
    {
        ...
        if(flag)
        {
            ...
            if(dt>3)
            {
                ...
                Debug.Log(marker_list[marker_num]);
                WorldMarkers.key = marker_list[marker_num];
                WorldMarkers.keyflag =true;

                // Sound as the marker moves
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.Space))
        {
            ...
            {
                flag = true;
                gaze_flag = marker_list[marker_num];
                init_time = Time.time;
                audioSource.PlayOneShot(sound1);
            }
        }

        if (actionToTrigger.GetStateDown(SteamVR_Input_Sources.RightHand))
        {
            if(!flag)
            {
                flag = true;
                ...
                // Sound as the user starts gazing
                audioSource.PlayOneShot(sound1);
            }
        }
    }
